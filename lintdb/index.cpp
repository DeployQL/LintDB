...
namespace lintdb {
    ...
    void IndexIVF::train(float* embeddings, size_t n, size_t dim) {
        // TODO(mbarta): this can follow faiss' example and be stored inside its own index.
        try {
    ...
    std::vector<SearchResult> IndexIVF::search(EmbeddingBlock& block, size_t n_probe, size_t k) const {       
        ...
        quantizer->search(block.num_tokens, block.embeddings.data(), k_top_centroids, distances.data(), coarse_idx.data());
...
#pragma omp parallel
{
        ...
        for(size_t i = 0; i < block.num_tokens; i++) {
            ...
            // our iterator abstraction was not working. The upper bound key was being
            // destroyed during iteration, which causes rocksdb to throw an error.
            // for now, we use a raw rocksdb iteration.
            auto options = rocksdb::ReadOptions();
            rocksdb::Slice end_slice(end_string);
            options.iterate_upper_bound = &end_slice;
            auto it = std::unique_ptr<rocksdb::Iterator>(db->NewIterator(options, column_families[kIndexColumnIndex]));
        ...
        */
        ...
        std::vector<float> max_scores_per_centroid = max_score_by_centroid(coarse_idx, distances, k_top_centroids, block.num_tokens, nlist);
        std::vector<std::pair<float, idx_t>> pid_scores(pid_list.size());
        // create a mapping from pid to the index. we'll need this to hydrate residuals.
...
#pragma omp for
        for (int i = 0; i < pid_list.size(); i++) {
            ...
            VLOG(100) << "approximate score for pid: " << pid_list[i] << " is: " << score;
        ...
        std::sort(pid_scores.begin(), pid_scores.end(), std::greater<std::pair<float, idx_t>>());
        auto top_25 = std::max(size_t(1), pid_scores.size() / 4);
        
        VLOG(100) << "top 25%: " << top_25;
        std::vector<std::pair<float, idx_t>> top_25_scores(pid_scores.begin(), pid_scores.begin() + top_25);

        /**
         * score by passage residuals
        */
        std::vector<idx_t> top_25_ids;
        std::transform(top_25_scores.begin(), top_25_scores.end(), std::back_inserter(top_25_ids), [](std::pair<float, idx_t> p) {
            return p.second;
        });
...
#pragma omp for
        for (int i = 0; i < top_25_ids.size(); i++) {
            ...
            VLOG(100) << "actual score for pid: " << top_25_ids[i] << " is: " << score;
            actual_scores[i] = std::pair<float, idx_t>(score, top_25_ids[i]);
        }
        VLOG(100) << "got actual scores: " << actual_scores.size();
        // according to the paper, we take the top 25%.
        std::sort(actual_scores.begin(), actual_scores.end(), std::greater<std::pair<float, idx_t>>());
        
        size_t num_to_return = std::min<size_t>(actual_scores.size(), k);
        std::vector<std::pair<float, idx_t>> top_k_scores(actual_scores.begin(), actual_scores.begin() + num_to_return);

        std::vector<SearchResult> results;
        std::transform(top_k_scores.begin(), top_k_scores.end(), std::back_inserter(results), [](std::pair<float, idx_t> p) {
    ...
    std::unique_ptr<EncodedDocument> IndexIVF::encode_vectors(const RawPassage& doc) const {
        LINTDB_THROW_IF_NOT(nlist <= std::numeric_limits<code_t>::max());
        ...
        return std::make_unique<EncodedDocument>(EncodedDocument(token_coarse_idx, residual_codes, num_tokens, doc.id, doc.doc_id));
    }

    std::vector<float> IndexIVF::decode_vectors(gsl::span<code_t> codes, gsl::span<residual_t> residuals, size_t num_tokens, size_t dim) const {
        ...
        for (size_t i = 0; i < num_tokens; i++) {
            ...
            binarizer->sa_decode(1, residuals.data() + i * binarizer->sa_code_size(), decoded_residuals.data());
