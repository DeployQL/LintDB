...
namespace lintdb {
    ...
    std::unique_ptr<Iterator>  RocksDBInvertedList::get_iterator(const std::string& start, const std::string& end) const {
        return std::make_unique<RocksDBIterator>(RocksDBIterator(db_, column_families[kIndexColumnIndex], start, end));
    }
    ...
    void RocksDBInvertedList::add(std::unique_ptr<EncodedDocument> doc){
        ...
        // get unique indexes.
        std::unordered_set<idx_t> unique_coarse_idx(doc->codes.begin(), doc->codes.end());
        ...
        for (code_t idx : unique_coarse_idx) {
            ...
            rocksdb::Status status = db_.Put(
                ...
                rocksdb::Slice() // store nothing. we only need the key to tell us what documents exist.
            );
            assert(status.ok());
            VLOG(100) << "Added document with id: " << doc->id << " to inverted list " << idx;
        }
        ...
        std::vector<idx_t> unique_coarse_idx_vec(unique_coarse_idx.begin(), unique_coarse_idx.end());
        auto mapping_ptr = create_doc_mapping(
            unique_coarse_idx_vec.data(),
            unique_coarse_idx_vec.size()
        );
        ...
        rocksdb::Status mapping_status = db_.Put(
            ...
            rocksdb::Slice(fks),
            rocksdb::Slice(reinterpret_cast<const char*>(mapping_ptr->GetBufferPointer()), mapping_ptr->GetSize())
        );
    ...
    std::vector<std::unique_ptr<EncodedDocument>> RocksDBInvertedList::get_residuals(std::vector<idx_t> ids) const {
        std::vector<rocksdb::Slice> keys;
        // rocksdb slices don't take ownership of the underlying data, so we need to keep the strings around.
        ...
        for (idx_t i=0; i<ids.size(); i++) {
            ...
            keys.push_back(rocksdb::Slice(key_strings[i].data(), key_strings[i].size()));
        }

        assert(keys.size() == ids.size());
        VLOG(100) << "Getting num docs: " << keys.size() << " from the forward index.";
    ...
    std::vector<std::unique_ptr<EncodedDocument>> RocksDBInvertedList::get_codes(std::vector<idx_t> ids) const {
        std::vector<rocksdb::Slice> keys;
        // rocksdb slices don't take ownership of the underlying data, so we need to keep the strings around.
        std::vector<std::string> key_strings;
        for (idx_t i=0; i<ids.size(); i++) {
        ...
        for (size_t i = 0; i < ids.size(); i++) {
            VLOG(100) << "Status of document with id: " << ids[i] << " is: " << statuses[i].ToString();
            if (statuses[i].ok()) {
