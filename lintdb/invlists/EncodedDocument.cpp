...
namespace lintdb {
    EncodedDocument::EncodedDocument(
        const std::vector<code_t> c, //reflects the centroid id for each token vector.
        const std::vector<residual_t> r, // reflects the residual vector for each token vector.
        size_t num_tokens,
        idx_t id, 
        std::string doc_id
    ): codes(c), 
