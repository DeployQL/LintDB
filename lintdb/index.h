...
struct IndexIVF {
    ...
    */
    std::vector<SearchResult> search(EmbeddingBlock& block, size_t n_probe, size_t k) const;
    ...
    */
    std::unique_ptr<EncodedDocument> encode_vectors(const RawPassage& doc) const;

    /**
     * Decode vectors translates out of our internal representation. 
     * 
     * Note: The interface to this has been changing -- it depends on the structures we
     * use to retrieve the data, which is still in flux.
    */
