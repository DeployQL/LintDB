...
Collection::Collection(IndexIVF* index, const CollectionOptions& opts) {
    ...
    LINTDB_THROW_IF_NOT_MSG(
            index->config.dim == model->get_dims(),
            "model dimensions don't match index dimensions");

    bool is_xtr = index->config.quantizer_type == IndexEncoding::XTR;
    this->tokenizer = std::make_unique<Tokenizer>(
            opts.tokenizer_file, opts.max_length, is_xtr);
}
...
void Collection::add(
    ...
    auto passage = EmbeddingPassage(
            output.data(), ids.size(), model->get_dims(), id, metadata);
    index->add(tenant, {passage});
}

void Collection::add_batch(
        const uint64_t tenant,
        const std::vector<TextPassage>& passages) const {
...
std::vector<SearchResult> Collection::search(
    ...
    //        normalize_vector(query_data.data(), ids.size(),
    //        model->get_dims());
