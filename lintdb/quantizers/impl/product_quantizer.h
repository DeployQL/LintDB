...
class ProductQuantizer {
   public:
    ProductQuantizer(
        ...
        if (d % m != 0) {
            throw std::invalid_argument(
                    "The dimensionality must be divisible by the number of sub-quantizers.");
        }
    }
    ...
    void fit(const std::vector<std::vector<float>>& data) {
        ...
        for (size_t i = 0; i < m; ++i) {
            ...
            for (size_t j = 0; j < data.size(); ++j) {
                std::copy(
                        flattened_data.begin() + j * d + i * subvector_dim,
                        flattened_data.begin() + j * d +
                                (i + 1) * subvector_dim,
            ...
            std::copy(
                    centroids.begin(),
                    centroids.end(),
                    codebooks.begin() + i * k * subvector_dim);
        }
    ...
    std::vector<uint8_t> quantize(const std::vector<float>& vec) const {
        if (vec.size() != d) {
            throw std::invalid_argument(
                    "Vector size does not match dimensionality.");
        }
        ...
        for (size_t i = 0; i < m; ++i) {
            gsl::span<const float> subvec(
                    vec.data() + i * subvector_dim, subvector_dim);
    ...
    std::vector<float> decode(const std::vector<uint8_t>& codes) const {
        if (codes.size() != m) {
            throw std::invalid_argument(
                    "Codes size does not match number of sub-quantizers.");
        }
        ...
        for (size_t i = 0; i < m; ++i) {
            gsl::span<const float> centroid(
                    codebooks.data() + (i * k + codes[i]) * subvector_dim,
                    subvector_dim);
            std::copy(
                    centroid.begin(),
                    centroid.end(),
                    vec.begin() + i * subvector_dim);
        }

        return vec;
    }

    std::vector<std::vector<uint8_t>> compute_codes(
    ...
    void serialize(const std::string& filename) const {
        ...
        ofs.write(
                reinterpret_cast<const char*>(&metric_value),
                sizeof(metric_value));

        // Write codebooks
        ofs.write(
                reinterpret_cast<const char*>(codebooks.data()),
    ...
    void deserialize(const std::string& filename) {
        ...
        // Read codebooks
        ifs.read(
                reinterpret_cast<char*>(codebooks.data()),
                codebooks.size() * sizeof(float));
   ...
   private:
    ...
    uint8_t find_nearest_codebook_index(
            gsl::span<const float> subvec,
            size_t subquantizer_index) const {
        float best_metric = (metric == Metric::EUCLIDEAN)
        ...
        for (size_t i = 0; i < k; ++i) {
            gsl::span<const float> centroid(
                    codebooks.data() +
                            (subquantizer_index * k + i) * subvector_dim,
                    subvector_dim);
