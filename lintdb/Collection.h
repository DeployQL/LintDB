...
/**
 * Collection is a collection of documents. Instead of dealing directly with
 * vectors, this class allows you to add and search for documents by text.
 */
struct Collection {
     ...
     * @param metadata a dictionary of metadata to store with the document. only
     * accepts strings.
     */
    void add(
    ...
    void add_batch(
            const uint64_t tenant,
            const std::vector<TextPassage>&
                    passages // pass by reference has some issues with swig
                             // here.
    ) const;
    /**
