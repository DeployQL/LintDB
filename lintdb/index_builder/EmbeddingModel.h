...
namespace lintdb {
    ...
    /**
     * BatchOutput is a helper struct that allows us to extract the output of a batched model.
     * 
    ...
    */
    struct BatchOutput {
        ...
        BatchOutput(const std::vector<float>& data, const std::vector<int64_t>& shape, const std::vector<size_t>& dims)
            : data(data), shape(shape), dims(dims) {}

        std::vector<float> get(int i) const {
            std::vector<float> output;

            auto starting_point = i * shape[1] * shape[2]; // i * max_len * embedding_dimension
            // dims[i] is the original length of the sequence. shape[2] is the embedding dimension.
            // this gives us the N token embeddings without padding.
            output.insert(output.end(), data.begin() + starting_point, data.begin() + starting_point + dims[i] * shape[2]);
            return output;
        }

        private:
        std::vector<float> data; /// [batch_size, max_len, embedding_dimension]. data that needs to be trimmed.
