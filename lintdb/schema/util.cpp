...
std::unique_ptr<flatbuffers::FlatBufferBuilder> create_forward_index_document(
    ...
    auto doc = CreateForwardIndexDocument(
            *builder, fb_doc_id, num_tokens, fb_residuals);
    builder->Finish(doc);
...
std::unique_ptr<flatbuffers::FlatBufferBuilder> create_doc_mapping(
    ...
    std::transform(
            coarse_idx,
            coarse_idx + idx_size,
            std::back_inserter(casted_codes),
            [](idx_t i) { return static_cast<uint32_t>(i); });

    auto builder = std::make_unique<flatbuffers::FlatBufferBuilder>();
    // auto codes = builder->CreateVector(flat_codes, num_tokens * code_size);
    auto fb_centroids =
            builder->CreateVector(casted_codes.data(), casted_codes.size());
