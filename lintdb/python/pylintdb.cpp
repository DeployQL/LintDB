...
// Helper function to convert std::chrono::time_point to a Python datetime object
...
// Wrapper functions
    ...
    SearchOptions create_search_options_from_dict(const nb::dict &dict) {
        ...
        if (dict.contains("centroid_score_threshold"))
            opts.centroid_score_threshold = nb::cast<float>(dict["centroid_score_threshold"]);
        ...
        if (dict.contains("nearest_tokens_to_fetch"))
            opts.nearest_tokens_to_fetch = nb::cast<size_t>(dict["nearest_tokens_to_fetch"]);
    ...
    Field PyField(const std::string &name, DataType data_type, const std::vector<FieldType> &field_types,
    ...
    IndexedField PyIndexedField(const std::string &name, DataType data_type, const nb::dict &params) {
        FieldParameters fp = create_field_parameters_from_dict(params);
        return IndexedField(name, data_type, fp);
    }

    ContextField PyContextField(const std::string &name, DataType data_type, const nb::dict &params) {
        FieldParameters fp = create_field_parameters_from_dict(params);
        return ContextField(name, data_type, fp);
    }

    StoredField PyStoredField(const std::string &name, DataType data_type, const nb::dict &params) {
        FieldParameters fp = create_field_parameters_from_dict(params);
        return StoredField(name, data_type, fp);
    }

    ColbertField PyColbertField(const std::string &name, DataType data_type, const nb::dict &params) {
        FieldParameters fp = create_field_parameters_from_dict(params);
    ...
    .value("QUANTIZED_TENSOR", DataType::QUANTIZED_TENSOR, "Quantized tensor data type")
    ...
    nb::class_<FieldParameters>(m,
    "FieldParameters", "Field parameters for configuration")
    .

    def(nb::init<>(),

    "Default constructor")
    .def_rw("dimensions", &FieldParameters::dimensions, "Number of dimensions")
    .def_rw("analyzer", &FieldParameters::analyzer, "Analyzer type")
    .def_rw("quantization", &FieldParameters::quantization, "Quantization type")
    .def_rw("num_centroids", &FieldParameters::num_centroids, "Number of centroids")
    .def_rw("num_iterations", &FieldParameters::num_iterations, "Number of iterations")
    .def_rw("num_subquantizers", &FieldParameters::num_subquantizers, "Number of subquantizers")
    .def_rw("nbits", &FieldParameters::nbits, "Number of bits");

    nb::class_<Field>(m,
    "__Field", "Field configuration")
    .
    ...
    def(nb::init<const std::string &, const DataType, std::vector<FieldType>, const FieldParameters &>(),
    ...
    // We don't expose these classes directly, but they get returned by wrapper functions.
    // Struct IndexedField
    nb::class_<IndexedField, Field>(m,
    "__IndexedField", "Indexed field configuration")
    .

    def(nb::init<const std::string &, const DataType, const FieldParameters &>(),

    "Constructor with parameters");

    // Struct ContextField
    nb::class_<ContextField, Field>(m,
    "__ContextField", "Context field configuration")
    .
    ...
    // Struct StoredField
    nb::class_<StoredField, Field>(m,
    "__StoredField", "Stored field configuration")
    .
    ...
    // Struct ColbertField
    nb::class_<ColbertField, Field>(m,
    "__ColbertField", "Colbert field configuration")
    .
    ...
    .def_static("fromJson", &Schema::fromJson, "Create schema from JSON")
    ...
    nb::class_<Duration>(m,
    "Duration", "Duration type");
    nb::class_<DateTime>(m,
    ...
    .def_static("from_python", &fromPythonDatetime, "Convert from Python datetime");

    // Struct ColBERTContextData
    nb::class_<ColBERTContextData>(m,
    "ColBERTContextData", "ColBERT context data")
    .

    def(nb::init<>(),

    "Default constructor")
    .def_rw("doc_codes", &ColBERTContextData::doc_codes, "Document codes")
    .def_rw("doc_residuals", &ColBERTContextData::doc_residuals, "Document residuals");

    // SupportedTypes
    nb::class_<SupportedTypes>(m,
    "SupportedTypes", "Supported data types");

    // Struct FieldValue
    nb::class_<FieldValue>(m,
    "FieldValue", "FieldValue for storing different types of data.")
    .
    ...
    def(nb::init<std::string, int>(), nb::arg("name"), nb::arg("value"),
    ...
    def(nb::init<std::string, float>(), nb::arg("name"), nb::arg("value"),
    ...
    def(nb::init<std::string, std::string>(), nb::arg("name"), nb::arg("value"),
    ...
    def(nb::init<std::string, DateTime>(), nb::arg("name"), nb::arg("value"),
    ...
    def(nb::init<std::string, Tensor>(), nb::arg("name"), nb::arg("value"),
    ...
    def(nb::init<std::string, Tensor, size_t>(), nb::arg("name"), nb::arg("value"), nb::arg("num_tensors"),
    ...
    def(nb::init<std::string, QuantizedTensor, size_t>(), nb::arg("name"), nb::arg("value"), nb::arg("num_tensors"),
    ...
    def(nb::init<std::string, ColBERTContextData, size_t>(), nb::arg("name"), nb::arg("value"), nb::arg("num_tensors"),
    ...
    .def_rw("num_tensors", &FieldValue::num_tensors, "Number of tensors.")
    ...
    m.def("TensorFieldValue", &TensorFieldValue, "Create FieldValue from Tensor");
    m.def("QuantizedTensorFieldValue", &QuantizedTensorFieldValue, "Create FieldValue from QuantizedTensor");
    m.def("DateFieldValue", &DateFieldValue, "Create FieldValue from DateTime");

    nb::class_<Document>(m,
    "Document", "Document for storing multiple fields and a unique ID.")
    .

    def(nb::init<idx_t, const std::vector<FieldValue> &>(), nb::arg("id"), nb::arg("fields"),
    ...
    .def_rw("fields", &Document::fields, "List of FieldValue objects in the document.");
    /**
     * Search Options and Indexes
     */
    nb::class_<SearchOptions>(m,
    "SearchOptions",
    ...
    .def_rw("metadata", &SearchResult::metadata, "Metadata for the document")
    .def_rw("token_scores", &SearchResult::token_scores, "Document token scores")
    .def("__lt__", &
    ...
    nb::class_<Configuration>(m,
    "Configuration", "Configuration for the index")
    .

    def(nb::init<>(),

    "Default constructor")
    .def_rw("lintdb_version", &Configuration::lintdb_version, "LintDB version")
    .def("__eq__", &
    Configuration::operator==,
    "Equality comparison operator");

    nb::class_<IndexIVF>(m,
    "IndexIVF", "IndexIVF is a multi-vector index with an inverted file structure.")
    .

    def(nb::init<const std::string &, bool>(), nb::arg("path"), nb::arg("read_only")
    ...
    def(nb::init<const std::string &, const Schema &, const Configuration &>(), nb::arg("path"), nb::arg(
            "schema"), nb::arg("config"),
    ...
    def(nb::init<const IndexIVF &, const std::string &>(), nb::arg("other"), nb::arg("path"),
    ...
    .def("train", &IndexIVF::train, nb::arg("docs"),
    ...
    .def("set_quantizer", &IndexIVF::set_quantizer, nb::arg("field"), nb::arg("quantizer"),
    "Set the quantizer for a field.\n\n"
    ":param field: The field to set the quantizer for.\n"
    ":param quantizer: The quantizer to set.")
    .def("set_coarse_quantizer", &IndexIVF::set_coarse_quantizer, nb::arg("field"), nb::arg("quantizer"),
    ...
    .def("search", [](IndexIVF &self, const uint64_t tenant, const Query &query, size_t k, const nb::dict &opts_dict) {
        SearchOptions opts = create_search_options_from_dict(opts_dict);
        return self.search(tenant, query, k, opts);
    }, nb::arg("tenant"), nb::arg("query"), nb::arg("k"), nb::arg("opts") = nb::dict(),
    ...
    .def("add", &IndexIVF::add, nb::arg("tenant"), nb::arg("docs"),
    ...
    .def("add_single", &IndexIVF::add_single, nb::arg("tenant"), nb::arg("doc"),
    "Add a single document to the index.\n\n"
    ":param tenant: The tenant to assign the document to.\n"
    ":param doc: The document to add.")
    .def("remove", &IndexIVF::remove, nb::arg("tenant"), nb::arg("ids"),
    ...
    .def("update", &IndexIVF::update, nb::arg("tenant"), nb::arg("docs"),
    "Update documents in the index. This is a convenience function for remove and add.\n\n"
    ":param tenant: The tenant the documents belong to.\n"
    ":param docs: The documents to update.")
    .def("merge", &IndexIVF::merge, nb::arg("path"),
...
.def_rw("read_only", &IndexIVF::read_only, "Flag indicating whether the index is read-only.");


// Enum QuantizerType
nb::enum_<QuantizerType>(m,
"QuantizerType", "Enumeration of quantizer types.")
.value("UNKNOWN", QuantizerType::UNKNOWN, "Unknown quantizer type.")
.value("NONE", QuantizerType::NONE, "No quantizer.")
.value("BINARIZER", QuantizerType::BINARIZER, "Binarizer quantizer.")
.value("PRODUCT_ENCODER", QuantizerType::PRODUCT_ENCODER, "Product encoder quantizer.");

// Bindings for Quantizer
nb::class_<Quantizer>(m,
"Quantizer", "Abstract Quantizer class providing an interface for quantization operations.")
.def("train", &Quantizer::train, nb::arg("n"), nb::arg("x"), nb::arg("dim"),
"Train the quantizer with the given data.\n\n"
":param n: Number of data points.\n"
":param x: Pointer to data.\n"
":param dim: Dimension size.")
.def("save", &Quantizer::save, nb::arg("path"),
"Save the quantizer to the specified path.\n\n"
":param path: Path to save the quantizer.")
.def("sa_encode", &Quantizer::sa_encode, nb::arg("n"), nb::arg("x"), nb::arg("codes"),
"Encode the given data using the quantizer.\n\n"
":param n: Number of data points.\n"
":param x: Pointer to data.\n"
":param codes: Pointer to encoded data.")
.def("sa_decode", &Quantizer::sa_decode, nb::arg("n"), nb::arg("codes"), nb::arg("x"),
"Decode the given data using the quantizer.\n\n"
":param n: Number of data points.\n"
":param codes: Pointer to encoded data.\n"
":param x: Pointer to decoded data.")
.def("code_size", &Quantizer::code_size,
"Get the size of the code.\n\n"
":return: Size of the code.")
.def("get_nbits", &Quantizer::get_nbits,
"Get the number of bits.\n\n"
":return: Number of bits.")
.def("get_type", &Quantizer::get_type,
...
def(nb::init<const std::vector<float> &, const std::vector<float> &, float, size_t, size_t>(),
        nb::arg("bucket_cutoffs"), nb::arg("bucket_weights"), nb::arg("avg_residual"), nb::arg("nbits"), nb::arg("dim"),
...
":param nbits: Number of bits.\n"
":param dim: Dimension size.")
.def("binarize", &Binarizer::binarize, nb::arg("residuals"),
...
.def("train", [](
Binarizer &self, nb::ndarray<float>
&x) {
self.
train(x
.shape(0), x.
...
.shape(1));
}, nb::arg("x"), "Train the Binarizer with the given data.")
.def("save", &Binarizer::save, nb::arg("path"),
"Save the Binarizer to the specified path.\n\n"
":param path: Path to save the Binarizer.")
.def("sa_encode", &Binarizer::sa_encode, nb::arg("n"), nb::arg("x"), nb::arg("codes"),
"Encode the given data using the Binarizer.\n\n"
":param n: Number of data points.\n"
":param x: Pointer to data.\n"
":param codes: Pointer to encoded data.")
.def("sa_decode", &Binarizer::sa_decode, nb::arg("n"), nb::arg("codes"), nb::arg("x"),
"Decode the given data using the Binarizer.\n\n"
":param n: Number of data points.\n"
":param codes: Pointer to encoded data.\n"
":param x: Pointer to decoded data.")
.def("code_size", &Binarizer::code_size,
"Get the size of the code.\n\n"
":return: Size of the code.")
.def("get_nbits", &Binarizer::get_nbits,
"Get the number of bits.\n\n"
":return: Number of bits.")
.def_static("load", &Binarizer::load, nb::arg("path"),
...
"ICoarseQuantizer", "Abstract ICoarseQuantizer class providing an interface for coarse quantization operations.")
.def("train", &ICoarseQuantizer::train, nb::arg("n"), nb::arg("x"), nb::arg("k"), nb::arg("num_iter"),
"Train the coarse quantizer with the given data.\n\n"
":param n: Number of data points.\n"
":param x: Pointer to data.\n"
":param k: Number of centroids.\n"
":param num_iter: Number of iterations.")
.def("save", &ICoarseQuantizer::save, nb::arg("path"),
"Save the coarse quantizer to the specified path.\n\n"
":param path: Path to save the quantizer.")
.def("assign", &ICoarseQuantizer::assign, nb::arg("n"), nb::arg("x"), nb::arg("codes"),
"Assign the nearest centroids to the given data points.\n\n"
":param n: Number of data points.\n"
":param x: Pointer to data.\n"
":param codes: Pointer to assigned codes.")
.def("sa_decode", &ICoarseQuantizer::sa_decode, nb::arg("n"), nb::arg("codes"), nb::arg("x"),
"Decode the given codes to data points.\n\n"
":param n: Number of data points.\n"
":param codes: Pointer to codes.\n"
":param x: Pointer to decoded data.")
.def("compute_residual", &ICoarseQuantizer::compute_residual, nb::arg("vec"), nb::arg("residual"), nb::arg("centroid_id"),
...
.def("compute_residual_n", &ICoarseQuantizer::compute_residual_n, nb::arg("n"), nb::arg("vec"), nb::arg("residual"), nb::arg("centroid_ids"),
...
.def("reconstruct", &ICoarseQuantizer::reconstruct, nb::arg("centroid_id"), nb::arg("embedding"),
"Reconstruct the embedding for a given centroid.\n\n"
":param centroid_id: Centroid ID.\n"
":param embedding: Pointer to reconstructed embedding.")
.def("search", &ICoarseQuantizer::search, nb::arg("num_query_tok"), nb::arg("data"), nb::arg("k_top_centroids"), nb::arg("distances"), nb::arg("coarse_idx"),
...
.def("reset", &ICoarseQuantizer::reset,
"Reset the coarse quantizer.")
.def("add", &ICoarseQuantizer::add, nb::arg("n"), nb::arg("data"),
"Add new data points to the coarse quantizer.\n\n"
":param n: Number of data points.\n"
":param data: Pointer to data points.")
.def("code_size", &ICoarseQuantizer::code_size,
"Get the size of the code.\n\n"
":return: Size of the code.")
.def("num_centroids", &ICoarseQuantizer::num_centroids,
"Get the number of centroids.\n\n"
":return: Number of centroids.")
.def("get_xb", &ICoarseQuantizer::get_xb,
"Get the centroids.\n\n"
":return: Pointer to centroids.")
.def("serialize", &ICoarseQuantizer::serialize, nb::arg("filename"),
"Serialize the coarse quantizer to a file.\n\n"
":param filename: File name to serialize to.")
.def("is_trained", &ICoarseQuantizer::is_trained,
...
.def("train", [](
CoarseQuantizer &self, nb::ndarray<float, nb::ndim<2>, nb::device::cpu>
&x,
size_t k, size_t
num_iter) {
self.
...
}, nb::arg("x"), nb::arg("k"), nb::arg("num_iter") = 10, "Train the CoarseQuantizer with the given data.")
.def("save", &CoarseQuantizer::save, nb::arg("path"),
"Save the quantizer to the specified path.\n\n"
":param path: Path to save the quantizer.")
.def("assign", &CoarseQuantizer::assign, nb::arg("n"), nb::arg("x"), nb::arg("codes"),
"Assign the nearest centroids to the given data points.\n\n"
":param n: Number of data points.\n"
":param x: Pointer to data.\n"
":param codes: Pointer to assigned codes.")
.def("sa_decode", &CoarseQuantizer::sa_decode, nb::arg("n"), nb::arg("codes"), nb::arg("x"),
"Decode the given codes to data points.\n\n"
":param n: Number of data points.\n"
":param codes: Pointer to codes.\n"
":param x: Pointer to decoded data.")
.def("compute_residual", &CoarseQuantizer::compute_residual, nb::arg("vec"), nb::arg("residual"), nb::arg("centroid_id"),
"Compute the residual vector for a given data point and centroid.\n\n"
":param vec: Pointer to data point.\n"
":param residual: Pointer to residual vector.\n"
":param centroid_id: Centroid ID.")
.def("compute_residual_n", &CoarseQuantizer::compute_residual_n, nb::arg("n"), nb::arg("vec"), nb::arg("residual"), nb::arg("centroid_ids"),
"Compute the residual vectors for multiple data points and centroids.\n\n"
":param n: Number of data points.\n"
":param vec: Pointer to data points.\n"
":param residual: Pointer to residual vectors.\n"
":param centroid_ids: Pointer to centroid IDs.")
.def("reconstruct", &CoarseQuantizer::reconstruct, nb::arg("centroid_id"), nb::arg("embedding"),
"Reconstruct the embedding for a given centroid.\n\n"
":param centroid_id: Centroid ID.\n"
":param embedding: Pointer to reconstructed embedding.")
.def("search", &CoarseQuantizer::search, nb::arg("num_query_tok"), nb::arg("data"), nb::arg("k_top_centroids"), nb::arg("distances"), nb::arg("coarse_idx"),
"Search for the nearest centroids to the given data points.\n\n"
":param num_query_tok: Number of query tokens.\n"
":param data: Pointer to data points.\n"
":param k_top_centroids: Number of top centroids to search.\n"
":param distances: Pointer to distances to nearest centroids.\n"
":param coarse_idx: Pointer to coarse indices of nearest centroids.")
.def("reset", &CoarseQuantizer::reset,
"Reset the quantizer.")
.def("add", &CoarseQuantizer::add, nb::arg("n"), nb::arg("data"),
"Add new data points to the quantizer.\n\n"
":param n: Number of data points.\n"
":param data: Pointer to data points.")
.def("code_size", &CoarseQuantizer::code_size,
"Get the size of the code.\n\n"
":return: Size of the code.")
.def("num_centroids", &CoarseQuantizer::num_centroids,
"Get the number of centroids.\n\n"
":return: Number of centroids.")
.def("get_xb", &CoarseQuantizer::get_xb,
"Get the centroids.\n\n"
":return: Pointer to centroids.")
.def("serialize", &CoarseQuantizer::serialize, nb::arg("filename"),
"Serialize the quantizer to a file.\n\n"
":param filename: File name to serialize to.")
.def_static("deserialize", &CoarseQuantizer::deserialize, nb::arg("filename"), nb::arg("version"),
...
// Version
nb::class_<Version>(m,
"Version", "Version class representing LintDB's version with major, minor, revision, and build numbers.")
.
...
.def_rw("metadata_enabled", &Version::metadata_enabled, "Flag indicating if metadata is enabled.")
 ...
 * Query nodes
 */
nb::enum_<QueryNodeType>(m,
"QueryNodeType", "Types of query nodes.")
...
nb::class_<QueryNode>(m,
"__QueryNode", "Base class for query nodes.");
...
//
//nb::class_<MultiQueryNode, QueryNode>(m, "__MultiQueryNode", "Multi query node.")
//.def(nb::init<QueryNodeType>(), nb::arg("op"),
//"Constructor with query node type.\n\n"
//":param op: Query node type.");
//
//nb::class_<VectorQueryNode, QueryNode>(m, "__VectorQueryNode", "Vector query node.")
//.def(nb::init<FieldValue&>(), nb::arg("value"),
//"Constructor with FieldValue.\n\n"
//":param value: FieldValue instance.");
//
//nb::class_<AndQueryNode, MultiQueryNode>(m, "__AndQueryNode", "AND query node.")
//.def(nb::init<std::vector<std::unique_ptr<QueryNode>>>(), nb::arg("its"),
//"Constructor with a list of child query nodes.\n\n"
...
.def(nb::init<std::unique_ptr<QueryNode>>(), nb::arg("root"),
