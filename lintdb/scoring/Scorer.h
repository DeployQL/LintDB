...
namespace lintdb {
    ...
    struct ScoredDocument {
        float score;
        idx_t doc_id;
        std::vector<DocValue> values; /// ScoredDocument takes ownership of the values, because
        /// we assume we are iterating over a DocIterator and the values are only valid for the
...
public:
    ...
    virtual ScoredDocument score(QueryContext &context, idx_t doc_id, std::vector<DocValue>& fvs) const = 0;
};

class PlaidScorer: public Scorer {
public:
    explicit PlaidScorer(const QueryContext& context);
    ScoredDocument score(QueryContext &context, idx_t doc_id, std::vector<DocValue>& fvs) const override;
