...
namespace lintdb {
    ...
    std::unique_ptr<Binarizer> Binarizer::load(std::string path) {
        ...
        std::unique_ptr<Binarizer> binarizer = std::make_unique<Binarizer>(nbits, dim);
    ...
    std::vector<uint8_t> Binarizer::unpackbits(const std::vector<uint8_t>& packed, size_t dim, size_t nbits) {
        ...
        for (size_t i = 0; i < dim * nbits; ++i) {
            ...
            // Determine bit position in big-endian format
            size_t bigEndianBitOffset = 7 - bitOffset;
            uint8_t bit = (packed[byteIndex] >> bigEndianBitOffset) & 1; // Extract the corresponding bit
    ...
    std::vector<uint8_t> Binarizer::binarize(const std::vector<float>& residuals) {
        ...
        for (size_t i = 0; i < residuals.size(); ++i) {
            ...
            // If the residual is larger than all bucket_cutoffs, assign it to the last bucket
            if (!bucket_found) {
