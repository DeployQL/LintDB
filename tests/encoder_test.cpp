...
TEST(EncoderTest, ResidualsAreEncodedCorrectly) {
    ...
    std::shared_ptr<lintdb::Quantizer> quantizer =
            create_quantizer(lintdb::IndexEncoding::BINARIZER, qc);
    ...
    EXPECT_EQ(
            encoded_doc->residuals.size(),
            num_tokens * dim /
                    (8 / binarize_bits)); // 100 tokens * 128 dim / 4 dimensions
                                          // per byte = 3200 bytes.
}
...
TEST(EncoderTest, NoCompressionWorksCorrectly) {
    ...
    lintdb::DefaultEncoder encoder(
            2, 2, 2, 128, 0, lintdb::IndexEncoding::NONE);
    ...
    // our encoded residuals should be the same size they were before encoding,
    // taking the float size into account.
    EXPECT_EQ(encoded_doc->residuals.size(), num_tokens * dim * sizeof(float));

    auto decoded_doc = encoder.decode_vectors(
            encoded_doc->codes, encoded_doc->residuals, num_tokens);
