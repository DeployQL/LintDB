...
TEST(PlaidTests, ReadsCodesCorrectly) {
    // query scores reflect the max scores per centroid for all query terms
    // we've seen. we'll use 5 centroids.
    ...
    auto score = lintdb::score_documents_by_codes(query_scores, doc_codes, 0.0);
...
TEST(PlaidTests, CodesSameAsColBERT) {
    ...
    auto colbert = [](std::vector<code_t> codes,
    ...
    // we have a list of centroid scores for each token. this should be
    // (n_centroids, nquery_vectors)
    ...
    auto max_scores = lintdb::max_score_by_centroid(
            coarse_idx,
            centroid_scores,
            5, // k_per_token. multiplied with num_tokens to get the full length
               // of centroid_scores.
            5, // num_tokens
            5  // num_centroids
    );
    auto actual = lintdb::score_documents_by_codes(max_scores, doc_codes, 0.0);
...
TEST(PlaidTests, ResidualScoresTheSame) {
    ...
    auto colbert = [](std::vector<float> scores,
                      size_t doc_length,
                      size_t nquery_vectors) {
    ...
    auto matmul_transposedB = [](const std::vector<float>& A,
                                 int m,
                                 int k,
    ...
    // imagine we calculated our scores, creating an n_doc_token x n_query_token
    // matrix.
    ...
    auto scores = matmul_transposedB(
            doc_vectors,
            num_doc_tokens,
            dim,
            query_vectors,
    ...
    auto actual = lintdb::score_document_by_residuals(
            query_vectors,
            num_query_tokens,
            doc_vectors.data(),
            num_doc_tokens,
