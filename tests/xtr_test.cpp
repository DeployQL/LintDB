...
TEST(XTRTest, InitializesCorrectly) {
    ...
    auto retriever =
            lintdb::XTRRetriever(index, forward, encoder, product_quantizer);
}
...
TEST(XTRTest, GetDocumentScores) {
    ...
    lintdb::XTRRetriever::get_document_scores(
            n, all_doc_codes, document_scores, lowest_query_scores);
...
TEST(XTRRetrieverTest, ImputeMissingScores) {
    ...
    lintdb::XTRRetriever::impute_missing_scores(
            n, document_scores, lowest_query_scores);
